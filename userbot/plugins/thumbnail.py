...
"""Küçük Resim Yardımcı Programları, © @AnyDLBot
Available Commands:
.savethumbnail
.clearthumbnail
.getthumbnail"""
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit(
            "özel video  / dosya küçük resmi kaydedildi. " +
            "This image will be used in the upload, till `.clearthumbnail`."
        )
    else:
        await event.edit("Özel küçük resmi kaydetmek için fotoğrafı yanıtla")
...
async def _(event):
    ...
    await event.edit("✅ Özel  küçük resim başarıyla temizlendi.")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        caption_str = "Şu anda kaydedilmiş küçük resim temizleme `.clearthumbnail`"
        await borg.send_file(
            event.chat_id,
            thumb_image_path,
            caption=caption_str,
        ...
        await event.edit("bir medya ya yanıt olarak `.gethumbnail` cevapla")
