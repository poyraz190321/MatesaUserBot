...
async def matesalst_of_files(path):
    files = []
...
async def uploadir(event):
    ...
    udir_event = await edit_or_reply(event, "Yükleniyor...")
    if os.path.exists(input_str):
        await udir_event.edit(f"Dosya ayrıntılarını `{input_str}` dizininde toplama")
        lst_of_files = []
        lst_of_files = await matesalst_of_files(input_str)
        uploaded = 0
        await udir_event.edit(
            "{} Dosya bulundu. Yükleme yakında başlayacak. Lütfen bekle!".format(
                len(lst_of_files)))
            ...
            if os.path.exists(single_file):
                ...
                if not caption_rts.lower().endswith(".mp4"):
                    ...
                    await borg.send_file(
                        ...
                        progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                            progress(d, t, udir_event, c_time, "Yükleniyor...",
                                     single_file)))
                uploaded = uploaded + 1
        await udir_event.edit("{} Dosya başarıyla yüklendi !!".format(uploaded))
    else:
        await udir_event.edit("404: Dizin Bulunamadı")
...
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
    mone = await edit_or_reply(event, "İşleniyor...")
    thumb = None
    ...
    if os.path.exists(input_str):
        ...
        caat = await bot.send_file(
            ...
            thumb=thumb,
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, mone, c_time, "yüklemeye çalışıyor")
            )
        )
        end = datetime.now()
        ms = (end - start).seconds
        await mone.delete()
        await caat.edit(rf"__ ** ➥ {ms} saniye içinde yüklendi. ** __ \ n __ ** ➥ Yükleyen: - ** __ {DEFAULTUSER}")
    else:
        await mone.edit("404: Dosya Bulunamadı")
...
def get_video_thumb(file, output=None, width=320):
    ...
    p = subprocess.Popen([
        'ffmpeg', '-i', file,
        '-ss', str(int((0, metadata.get('süresi').seconds)
                       [metadata.has('süresi')] / 2)),
...
def extract_w_h(file):
    """Medyanın genişliğini ve yüksekliğini alın"""
    command_to_run = [
        "ffprobe",
        "-v",
        "quiet",
...
async def uploadas(event):
    ...
    uas_event = await edit_or_reply(event, "yükleniyor...")
        ...
        try:
            if supports_streaming:
                ...
                await borg.send_file(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, uas_event, c_time, "Yükleniyor...",
                                 file_name)))
            elif spam_big_messages:
                await uas_event.edit("TBD: (Henüz) Uygulanmadı")
                return
            try:
                os.remove(vthumb)
            except BaseException:
                pass
            await uas_event.edit("Başarıyla yüklendi !!")
...
\nUsage: Yerel olarak depolanan bir dosyayı sohbete yükler."
})
