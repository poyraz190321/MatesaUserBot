...
async def tor_search(event):
    ...
    for div in source.find_all('div', {'class': 'grey_bar3 back_none'}):
        ...
        counter = counter + 1
    if not urls:
        await event.edit("Anahtar Kelime kısıtlandı veya bulunamadı ..")
        return
    for url in urls:
...
async def _(event):
    ...
    await event.edit(
        f"Hurdaya {input_type} için {input_str} içinde {ms} saniye. Elde edilen sonuçlar: \n {output_str}",
        link_preview=False,
        parse_mode="html"
    )
