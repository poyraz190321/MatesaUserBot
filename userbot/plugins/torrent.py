...
async def remove_all(event):
    ...
    await event.edit("`Tüm İndirmeler Kaldırıldı.`")
...
async def pause_all(event):
    ...
    await event.edit("Çıktı: " + str(paused))
...
async def resume_all(event):
    ...
    await event.edit("Çıktı: " + str(resumed))
...
async def show_all(event):
    ...
    if len(msg) <= 4096:
        await event.edit("`Güncel İndirmeler: `\n" + msg)
    else:
        await event.edit("`Çıktı çok büyük. Dosya olarak gönderme...`")
