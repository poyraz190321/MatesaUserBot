...
async def waifu(animu):
    ...
    if not text:
        if animu.is_reply:
            ...
            await animu.edit("`Herhangi bir makale yazmadınız, Waifu gidiyor.`")
            return
    try:
...
async def sticklet(event):
    ...
    if not sticktext and event.reply_to_msg_id:
        ...
        await event.edit("bir şeye ihtiyacım var, hmm")
        return
    ...
    await event.client.send_file(event.chat_id, image_stream, caption="matesa's Sticklet", reply_to=event.message.reply_to_msg_id)
    # cleanup
    try:
        os.remove(FONT_FILE)
    except BaseException:
...
CMD_HELP.update({
    'stickerfun':
    "**📌Komut ➥**`.sttxt` <metin>\nUSAGE :Yazınızı eğlenceli hale getiren anime.\
    \n\n`.stcr` <metin>\n**Kullanım ➥ **etiket olarak metniniz\
    "
})
